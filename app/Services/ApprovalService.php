...
use App\Models\User;
use App\Notifications\BarangSedangDibeli;
use App\Notifications\BarangSudahMasuk;
...
class ApprovalService
{
    public function submit(Pengajuan $p, User $u): Pengajuan
    {
        ...
        if ($p->details()->count() === 0)
            throw ValidationException::withMessages(['detail' => 'Minimal satu barang harus ditambahkan.']);

        return DB::transaction(function () use ($p, $u) {
            $p->hitungTotalNilai();
            $this->log($p, $u, 'submit', StatusPengajuan::Diajukan, '', [
    ...
    public function review(Pengajuan $p, User $u, string $c = ''): Pengajuan
    {
        ...
        return DB::transaction(fn() =>
            tap($p, fn() => $this->log($p, $u, 'review', StatusPengajuan::ReviewAdmin, $c))
        );
    }
    ...
    public function teruskanKePurchasing(Pengajuan $p, User $u, string $c = ''): Pengajuan
    {
        ...
        return DB::transaction(function () use ($p, $u, $c) {
            $this->log($p, $u, 'teruskan', StatusPengajuan::Diteruskan, $c, [
                'jalur_approval' => $p->tentukanJalurApproval(),
            ]);
            return $p;
        });
    }
    ...
    public function prosesPurchasing(Pengajuan $p, User $u, string $c = ''): Pengajuan
    {
        ...
        return DB::transaction(fn() =>
            tap($p, fn() => $this->log($p, $u, 'proses', StatusPengajuan::ProsesPurchasing, $c))
        );
    }
    ...
    public function ajukanApprovalAkhir(Pengajuan $p, User $u, string $c = ''): Pengajuan
    {
        ...
        return DB::transaction(fn() =>
            tap($p, fn() => $this->log($p, $u, 'proses', StatusPengajuan::MenungguApproval, $c))
        );
    }
    ...
    public function konfirmasiTerima(Pengajuan $p, User $u, string $c = ''): Pengajuan
    {
        ...
        return DB::transaction(fn() =>
            tap($p, fn() => $this->log($p, $u, 'konfirmasi_terima', StatusPengajuan::Diterima, $c))
        );
    }
    ...
    {
        return DB::transaction(fn() =>
            tap($p, fn() => $this->log($p, $u, 'selesai', StatusPengajuan::Selesai, $c))
        );
    }
    ...
    private function ensure(Pengajuan $p, StatusPengajuan $exp): void
    {
        if ($p->status !== $exp)
            throw ValidationException::withMessages([
                'status' => "Aksi tidak dapat dilakukan. Status saat ini: {$p->status->label()}",
            ]);
    }

    private function ensureAdminDivisi(Pengajuan $p, User $u): void
    {
        if (!$u->isSuperadmin() && $u->divisi_id !== $p->divisi_id)
            throw ValidationException::withMessages([
                'divisi' => 'Anda tidak berwenang atas pengajuan dari divisi lain.',
            ]);
    }
    ...
    private function ensureApprover(Pengajuan $p, User $u): void
    {
        ...
        if (!$valid)
            throw ValidationException::withMessages([
                'approver' => 'Anda tidak memiliki wewenang untuk menyetujui pengajuan ini.',
            ]);
    }
}